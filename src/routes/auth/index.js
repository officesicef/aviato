...
const { User, PasswordRecovery, Contact } = require('models');
...
router.get('/self',
  async (req, res) => {
    try {
      const user = decrypt(req.headers.authorization);

      const returnUser = await User.findById(user.id, { raw: true });
      const contact = await Contact.findAll({
        where: { userId: user.id},
        limit: 1,
      });

      returnUser.emergency =  contact ? contact[0].number : null;
